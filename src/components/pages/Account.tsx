...
/** Props del componente {@link Account}. */
 ...
 *
 * @param currentUser - Perfil actual del usuario autenticado.
 * @param onBack - Navega hacia atrás.
 * @param onUpdate - Se ejecuta con el perfil actualizado tras guardar exitosamente.
 */
function Account({ currentUser, onBack, onUpdate }: AccountProps) {
  const { t } = useTranslation();
  const { fields, setters, handleSave, isSaving } = useAccountForm(currentUser, onUpdate);
 ...
 const handleAvatarChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  setters.setAvatarFile(file);    // guarda el File para subirlo al backend
};

  return (
    <BannerHeaderLayout title={t("account.title")} onBack={onBack}>
      ...
      <div className="px-5 sm:px-10 md:max-w-2xl md:mx-auto flex flex-col gap-4 py-6 pb-12">
        ...
        <AppButton text={t("account.save")} onClick={handleSave} disabled={isSaving} />
      </div>
